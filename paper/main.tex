...
\title{Chaotic states outperform quantum many-body scars as codes under
...
% (4) main result — "Here we show"
Here we show, by computing the reference-qubit coherent information of a logical
qubit encoded in the PXP scar subspace versus an energy-matched thermal
subspace under monitored dynamics, that thermal codes protect information
\emph{better} than PXP scar codes under every measurement we test.
% (5) comparison with prior work
The scar disadvantage is mild for local measurement, is largest for collective
measurements aligned with the emergent $\mathrm{su}(2)$ generator (which couples
the code states), and---decisively---survives even a fine-tuned Casimir
...
them no advantage.
% (7) broad outlook
They identify the code's spread in the measurement basis, not its athermality,
as the resource for protection, and caution that weak-ergodicity-breaking
...
untouched if and only if the code is an eigenspace of $O$ with a single shared
eigenvalue, so that every spectral projector of $O$ acts on the code as a
multiple of the identity. A measurement \emph{resolves}---and hence
dephases---the code through either of two channels: \emph{(i) eigenvalue
distinguishability}, when the logical states differ in their $O$-expectation or
carry a nonzero $O$-variance, so their outcome distributions
$P_a(o)=|\langle o|a_L\rangle|^2$ differ; or \emph{(ii) non-invariance}, when $O$
carries the code onto itself, $\langle0_L|O|1_L\rangle\neq0$, coherently mixing
the logical states. Equal mean and vanishing leak alone are insufficient---a
residual variance already realizes channel (i)---so a decoherence-free subspace
requires the logical states to be $O$-eigenstates sharing one eigenvalue. This
condition is independent of the microscopic model and organizes the results
below: thermal states, spread broadly and near-identically in a local basis, are
close to optimal codes, while scars are penalized whenever the measured operator
resolves their atypical structure through either channel.
...
\begin{figure}[t]
\includegraphics[width=\linewidth]{corrected_crossover.png}
...
ensemble) under local-$Z$ (left) and collective staggered-$Z$ (right) monitoring
...
channels: $-0.03$ to $-0.06$ (local) and $-0.25$ to $-0.32$ (collective), the
band excluding zero. At the largest size $L=18$ (constrained dimension $6765$)
it is $-0.05$ (local) and $-0.30$ (collective), essentially size-independent:
the refutation is not a finite-size artifact.
...
\begin{figure}[t]
\includegraphics[width=\linewidth]{extensive.png}
...
$M=\sum_i(-1)^iZ_i$---the emergent $\mathrm{su}(2)$ spectrum-generating
generator~\cite{Choi2019}---which carries a large logical leak
...
$\CR$ (Fig.~\ref{fig:mech}, right): every high-leak code is poorly protected,
...
both is a weak monotone trend (Fig.~\ref{fig:mech}, left): protection rises with
...
$Z$-basis participation (left) and versus logical leak $|\langle0|O|1\rangle|$
(right) over an eigenstate sweep (blue), product states (orange), and the
...
irreducible Casimir variance ($\mathrm{Var}\,J^2=32.5$ versus $18.1$ for the
thermal state), so $J^2$ measurement collapses them more strongly than it does
thermal states. No Casimir measurement can be a true DFS for approximate scars.
...
(Fig.~\ref{fig:spin1}, right). This flips the Casimir rescue
(Fig.~\ref{fig:spin1}, left): the exact-su(2) scar code is now a genuine
...
$\CR^{\rm scar}=1$ at every $p$, giving $\Delta\CR=+0.6$ to $+0.8$
...
Casimir probe: under generic local $S^z$ monitoring the exact-scar and thermal
codes are statistically comparable ($|\Delta\CR|\lesssim0.1$), so scar structure
still confers no advantage against a generic local measurement.

\begin{figure}[t]
\includegraphics[width=\linewidth]{spin1_rescue.png}
...
Left: scar$-$thermal coherent information $\Delta\CR$ versus $p$ (error bars are
...
thermal codes are comparable ($|\Delta\CR|\lesssim0.1$, grey). Right: scar
...
$\gamma_c\simeq0.013$ reported for weak continuous monitoring of PXP
scars~\cite{Paviglianiti2025}; the factor $\sim6$ is consistent with the
projective-per-step versus continuous-weak protocol difference.
...
(collective) at $p=0.10$, up to $L=18$; the scar's emergent-su(2) generator supplies a logical leak
of $1.73$ that makes it especially fragile, and even a fine-tuned Casimir DFS
...
available (repository archived at a Zenodo DOI upon publication), enabling
one-command reproduction of every result reported here.
