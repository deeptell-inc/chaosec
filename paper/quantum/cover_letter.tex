...
\vspace{1em}
\noindent July 27, 2026

\vspace{1em}
\noindent To the Editors, \emph{Quantum}
...
We submit for your consideration the manuscript above (research article, two
authors, eleven pages including nine appendices that replace a separate
supplement) together with an openly available simulation package reproducing
every result.

\vspace{0.5em}

% Paragraph 1: the surprise (overturns a common intuition) + what it reveals
A natural intuition holds that quantum many-body scars---non-thermal,
...
We believe \emph{Quantum} is the right venue. The result speaks directly to the
journal's core readership working on monitored dynamics, quantum error
correction, and quantum memory; it is conceptual and general rather than
model-specific; and it matches \emph{Quantum}'s standards of openness and
verifiability---the manuscript is fully self-contained (appendices in place of a
supplement), an open package regenerates every figure with one command, the
data are archived with a DOI, and the literature claims have been verified
against primary sources. We hope the Editors will agree that overturning a
common intuition, extracting a design principle, and surfacing a shared
methodological hazard together make a useful and lasting contribution.
