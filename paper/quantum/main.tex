...
logical qubit encoded at the canonical PXP scar rungs lies below that of an
...
states, and survives the Casimir measurement for which the pair is a
nominal decoherence-free subspace: scars carry excess Casimir variance, and
only rungs purer than the physically accessed ones are rescued by a
multiplet-resolved Casimir.
% (6) generalized meaning
An exactly solvable spin-1 model anchors the zero-variance end point: only
zero-variance \emph{exact} scars are protected, their
local-measurement advantage decaying with rate (its size scaling open). The
eigenstate-thermalization-to-error-correction correspondence thus holds directly
in the monitored-circuit setting, and hybridized scarring does not aid
protection against generic measurement.
% (7) broad outlook
Scrambling is therefore the design resource for measurement-robust encoding, and
physically realized scar subspaces are liabilities rather than assets for
...
information better than the canonical scar code in every case; that an apparent scar advantage
...
decisions are additionally backed by a hierarchical bootstrap that resamples
thermal pairs and propagates per-code trajectory noise: at $p=0.10$ the
$95\%$ confidence intervals are $[-0.056,-0.024]$ (local) and
$[-0.317,-0.224]$ (collective) at $L=14$, and an independent-seed reanalysis
of the marginal $L=12$ local point gives $\Delta\CR=-0.027$,
$[-0.042,-0.012]$---all excluding zero
(\texttt{scripts/uncertainty\_audit.py}). At the largest size $L=18$
...
$-0.150$ (collective) against $-0.066$ and $-0.306$
...
The physically meaningful question is therefore posed at the resolution of
the emergent multiplets: binning $J^2$ outcomes to the nearest $j(j{+}1)$
turns the measurement into a five-outcome coarse probe under which a code
\emph{can} survive a shot, and Casimir variance becomes an operational knob.
Under this multiplet-resolved Casimir, with thermal pools cleaned of all
FSA-adjacent states (Appendix~\ref{app:ensemble}), the canonical rungs still
lose ($\Delta\CR=-0.01,-0.11,-0.17$ at $E=2.74$), while the purer rungs are
\emph{rescued}: $\Delta\CR=+0.18,+0.25,+0.20$ at $E=5.34$ and
$+0.02,+0.03,+0.04$ at $E=6.57$ (\texttt{scripts/pure\_rung\_dfs2.py}). Two
facts organize this: at every energy-matchable rung the scar retains excess
Casimir variance over its cleaned thermal neighbors [Var ratio
$6.9,4.8,4.4,3.1$ at $E\simeq2.7,4.1,5.3,6.6$; Var$\,J^2$ itself falls
$32.5\to8.6$ with FSA weight $0.53\to0.81$], yet survival also depends on
where the code's $\langle J^2\rangle$ sits relative to the bin edges, so the
rescue is \emph{not} a monotone function of variance alone. The honest
summary: a Casimir DFS is impossible at full spectral resolution, fails for
the hybridized canonical rungs even at multiplet resolution, and succeeds
for sufficiently pure rungs---hybridization with the thermal bulk, which is
maximal exactly where the physical tower is accessed, is what strips
approximate scars of their Casimir protection. The exactly solvable model
below supplies the clean end point where the variance vanishes identically
and the rescue is exact.
...
fails for the hybridized rungs at any spectral resolution---scars retain
excess Casimir variance over energy-matched thermal neighbors (ratio
$3.1$--$6.9$)---and only rungs far purer than the physically accessed ones
are rescued by a multiplet-resolved Casimir.
...
report $\CR$ as the time average of $S_R$ over the last quarter of the
recorded points of a $40$-step run ($S_R$ recorded every $4$ steps, so the
average covers the final two records, $t\in[21.6,24]$---the last $10\%$ of
the evolution window), averaged over $120$--$400$ Born-sampled trajectories with
